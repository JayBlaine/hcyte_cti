...
def pop_live_line_fig(flows:dict=None, y_ax:str='num_flows', interface:str=None):
    ...
    fig = px.line(
        data_frame=df, title='Live Flows: {} interface'.format(interface), hover_name='sec', hover_data=df.columns.tolist(), x='sec', y=y_ax).add_trace(
        px.line(x='alerts', y='sec')).update_xaxes(rangeslider_visible=True).update_layout(height=300)#.update_traces(hovertemplate='%{y}<br>%{text}')
    return fig
